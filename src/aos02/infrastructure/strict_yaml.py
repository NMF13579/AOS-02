...
from .schema_registry import validate_record_schema

MAX_DOCUMENT_BYTES = 1_048_576
MAX_NESTING_DEPTH = 32
MAX_COLLECTION_ITEMS = 10_000
KNOWN_RECORD_FILENAMES = frozenset({
    "idea.yaml", "risk.yaml", "scope.yaml", "task.yaml", "evidence.yaml",
    "execution-decision.yaml", "execution-request.yaml", "result-decision.yaml",
...
class _StrictSafeLoader(yaml.SafeLoader):
    ...
    def construct_mapping(self, node, deep=False):
        if not isinstance(node, MappingNode):
            raise yaml.constructor.ConstructorError(None, None, "expected a YAML mapping", node.start_mark)
        seen: set[str] = set()
        for key_node, _ in node.value:
            ...
            if not isinstance(key, str):
                raise yaml.constructor.ConstructorError(None, None, "YAML mapping keys must be strings", key_node.start_mark)
            if key in seen:
...
def load_records(directory: Path, required_records: tuple[str, ...]) -> dict[str, Any]:
    ...
    if unknown_requirements:
        raise BundleLoadError(f"unknown required record: {sorted(unknown_requirements)[0]}")
    entries: dict[str, Path] = {}
    collision_keys: set[str] = set()
    for entry in sorted(directory.iterdir(), key=lambda path: path.name.casefold()):
        if entry.is_symlink() or not entry.is_file():
            raise BundleLoadError(f"bundle entry must be a regular non-symlink file: {entry.name}")
    ...
    records = {filename.removesuffix(".yaml"): _load_mapping(path) for filename, path in entries.items()}
