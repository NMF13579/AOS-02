"""Read-only command-line interface for AOS-02 application use cases."""
...
from ..application.control_validation import validate_bundle
from ..application.decision_assessment import (
    validate_human_publication_decision,
    validate_human_result_decision,
)
...
from ..application.task_compilation import TaskBriefError, compile_task_brief
from ..infrastructure.strict_yaml import load_records
...
def main(argv: list[str] | None = None) -> int:
    ...
    execute_scoped = subcommands.add_parser("execute-scoped", help="disabled non-mutating execution endpoint")
    ...
    try:
        if args.command == "validate":
            ...
            result = compile_task_brief(idea=records["idea"], risk=records["risk"], scope=records["scope"], task_id=args.task_id, required_checks=args.check)
        elif args.command == "validate-result":
            records = load_records(args.bundle, RESULT_DECISION_RECORDS)
            result = validate_human_result_decision(task=records["task"], evidence=records["evidence"], decision=records["result-decision"])
        elif args.command == "validate-publication":
            records = load_records(args.bundle, PUBLICATION_DECISION_RECORDS)
            result = validate_human_publication_decision(task=records["task"], evidence=records["evidence"], decision=records["publication-decision"])
        elif args.command == "validate-execution":
            records = load_records(args.bundle, EXECUTION_DECISION_RECORDS)
            result = validate_human_execution_decision(task=records["task"], decision=records["execution-decision"])
        elif args.command == "preview-execution":
            records = load_records(args.bundle, EXECUTION_PREVIEW_RECORDS)
            result = preview_scoped_execution(task=records["task"], decision=records["execution-decision"], request=records["execution-request"])
        else:
            records = load_records(args.bundle, EXECUTION_PREVIEW_RECORDS)
            result = execute_scoped_request(root=args.root, task=records["task"], decision=records["execution-decision"], request=records["execution-request"])
